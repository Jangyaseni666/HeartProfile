...
//     document.getElementById("age_msg").innerText =
//       "Age must be between 20 to 79";
//   } else {
//     document.getElementById("age_msg").innerText = "";
//   }
// };
...
dform.addEventListener("submit", () => {
  ...
  var gnd = document.getElementById("sex");
  console.log(gnd);
  var gender = Array.from(gnd).find((radio) => radio.checked);
  var smk = document.getElementsByName("smoker");
  var smoker = Array.from(smk).find((radio) => radio.checked);
  var b_p = document.getElementsByName("bp");
  var bp = Array.from(b_p).find((radio) => radio.checked);
  console.log(
    // gender.value,
    age.value,
    totch.value,
  ...
  const details = {
    ...
    smoker: [smoker.value, smk_yes_[smk_yes_fact]],
  ...
  fetch("http://localhost:8082/details", {
    method: "POST",
    headers: {
